...
public class VerifyService {
	...
	public boolean verify(File arquivoAssinado, File[] cadeiaCertificados) {
		try (FileInputStream fis = new FileInputStream(arquivoAssinado)) {
			...
			X509Certificate certAssinante = new JcaX509CertificateConverter().setProvider("BC")
					.getCertificate(certHolder);

			boolean assinaturaValida = signer
					.verify(new JcaSimpleSignerInfoVerifierBuilder().setProvider("BC").build(certAssinante));
